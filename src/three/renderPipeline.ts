...
// Renderer, studio environment and post chain (HDR render → bloom → ACES tone map).
...
export function addStudioLights(scene: THREE.Scene) {
  ...
  const rim = new THREE.DirectionalLight('#8fd3ff', 1.7)
...
export function createComposer(renderer: THREE.WebGLRenderer, scene: THREE.Scene, camera: THREE.Camera, isHigh: boolean) {
  ...
  const bloom = new UnrealBloomPass(new THREE.Vector2(256, 256), 0.3, 0.42, 1.0)
